...
**Deviation 6 (2026-09-24): a per-subject-truncated HRE is added alongside the pre-registered HRE.**

**What was pre-registered.** §3, metric 4 and Methodology §3.3: HRE = mean over test subjects of the weighted L2 error of the hazard on a fixed grid up to tau, with weights w(t) proportional to the *pooled* marginal density of the true event-time distribution, the same w(t) for every subject.

**What was found.** In a review-2 diagnostic (S1, n = 5000, 20% Type I censoring, one untuned replication, four methods), about 93% of every method's HRE came from times at which the subject's own true CDF was already above 0.99. Mechanism: w(t) is pooled, so a high-risk subject, who has almost surely had the event by the marginal mode, still has their enormous hazard in the tail counted in full, although the data carry almost no information there. HRE then rewards whichever model has the right *parametric tail* (DSM and FlowSurv-Gumbel on S1, S4, S5), the "HRE rewards the true family" threat of Methodology §8. The diagnostic also ruled out the spline's identity tails (0% of HRE outside the spline bound; widening the bound from 4 to 8 barely moved HRE). That diagnostic's script is not in this repository; the claim is to be re-verified on the first grid output by the ratio of `hre` to `hre_trunc` and by `hre_cum`.

**Deviation.** A second hazard-recovery metric, `hre_trunc`, is stored next to `hre` (which is unchanged): for subject i the integral runs only over t <= Q_true(0.9 | x_i), the subject's own true 0.9-quantile (the first grid time with F_true >= 0.9, the whole grid if never reached), with the same pooled w(t) inside that window renormalized per subject, i.e. a weighted average squared hazard error over the subject's own window. Contrasts C1 and C3 are evaluated on **both** `hre` and `hre_trunc`; the exploratory cumulative-hazard companion `hre_cum` (Deviation 5) is unchanged. The truncation level (0.9, matching tau) was fixed here, before any result on the corrected pipeline exists.

**Why this is not post-hoc cherry-picking.** No grid result exists on the corrected pipeline (the only numbers are small untuned diagnostics), the definition and level are fixed in advance, both versions are reported for every confirmatory contrast, and no claim may rest on whichever one happens to favour FlowSurv-AFT: if the two disagree on a contrast, both are reported and the disagreement is stated.

**Consequence.** Rows written before this change have no `hre_trunc`. The per-fit prediction store (`flowsurv.eval.predictions`, `--predictions-dir`) was added at the same time so that a further estimator change is a recompute rather than a refit; it is on by default for simulation replications 1-5 and all real-data splits (about 0.4 MB per simulation fit; storing all 100 replications would need roughly 50 GB).

*Decision on the flow base distribution (2026-09-24).* Review 2 suggested a minimum-Gumbel base for FlowSurv-AFT (Weibull-AFT warm start) as a possible remedy for the S1 flexibility penalty. A tuned check (`scripts/s1_flexibility_check.py`; S1, n = 1000 and 5000, 20% Type I censoring; FlowSurv-AFT, FlowSurv-Gumbel, DSM, Weibull-AFT; 30-config tuning on reps 1-5, medians over reps 1-8; 64/64 fits converged) found: Weibull-AFT is best on every hazard and distribution metric; at n = 5000 FlowSurv-AFT / FlowSurv-Gumbel / DSM are 39 / 33 / 21 times worse than Weibull-AFT on `hre_trunc` and 4.9 / 3.5 / 3.1 times worse on KS; the Gumbel base gains only about 1.4 times over the logistic base on `hre_trunc` (1.15-1.39 on KS), and most of its apparent 4.4 times advantage on the pre-registered HRE disappears under the truncated HRE; IBS non-inferiority to Weibull-AFT holds (0.1296 vs 0.1267, margin 0.01), calibration and Uno's C are indistinguishable. **The base is kept logistic**, as pre-registered; no deviation is made. The flexibility cost on S1 is a finding to be reported for H3 (IBS non-inferior, hazard recovery not), and FlowSurv-Gumbel stays in the study as an ablation. Limits: 8 replications, one censoring setting, S1 only.
