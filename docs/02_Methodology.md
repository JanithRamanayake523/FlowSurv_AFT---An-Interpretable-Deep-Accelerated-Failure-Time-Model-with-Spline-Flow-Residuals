...
- Hyperparameter selection: validation NLL over the grid {K ∈ 1–3 RQS blocks} × {bins ∈ 8, 16} × {hidden ∈ 64, 128} × {dropout ∈ 0.1, 0.2} — 30 random configurations, identical budget for every DL baseline (fairness constraint). Selection is scored on the same validation split the fit early-stopped on, which is mildly optimistic for the chosen configuration; it is identical for every deep method, so the comparison stays fair.
